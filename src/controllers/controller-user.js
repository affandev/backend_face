...
let tokens = []
let refreshTokens = []
let batasjarak = 0.6608681952504225

function getDistanceBetween(latitude1, longitude1,unit) {
    console.log(latitude1, longitude1)
    const jsDistanceObj = new jsDistance(
        latitude1, // origin latitude
        longitude1, // origin longitude
        -6.36431243,  // kantor lat
        107.17192902 // kantor longtitude

    )
    let radius = unit === 'Mi' ? jsDistanceObj.count().toMiles() : jsDistanceObj.count().toKilometre()
...
module.exports = {
    ...
    getRadius(req, res) {
        ...
        kilometerjarak = getDistanceBetween(parseFloat(req.body.latitude), parseFloat(req.body.longtitude), 'Km')
    
        res.send({ hasil :kilometerjarak})
    },
    ...
    getJam(req, res) {
        let dt = new Date()
        let jam = `${(dt.getMonth() + 1).toString().padStart(2, '0')}-${dt.getDate().toString().padStart(2, '0')}-${dt.getFullYear().toString().padStart(4, '0')} ${dt.getHours().toString().padStart(2, '0')}:${dt.getMinutes().toString().padStart(2, '0')}`;
        res.send(jam)
    },
    isiAbsen(req, res) {
        console.log(req.lokasi)
        let keterangan
        let absensi
        let dt = new Date()
        let jam = `${(dt.getMonth() + 1).toString().padStart(2, '0')}-${dt.getDate().toString().padStart(2, '0')}-${dt.getFullYear().toString().padStart(4, '0')} ${dt.getHours().toString().padStart(2, '0')}:${dt.getMinutes().toString().padStart(2, '0')}`;
        let batas_pagi = `${(dt.getMonth() + 1).toString().padStart(2, '0')}-${dt.getDate().toString().padStart(2, '0')}-${dt.getFullYear().toString().padStart(4, '0')} 05:00`;
        let batas_absenpagi = `${(dt.getMonth() + 1).toString().padStart(2, '0')}-${dt.getDate().toString().padStart(2, '0')}-${dt.getFullYear().toString().padStart(4, '0')} 08:45`;
        let batas_sore = `${(dt.getMonth() + 1).toString().padStart(2, '0')}-${dt.getDate().toString().padStart(2, '0')}-${dt.getFullYear().toString().padStart(4, '0')} 15:00`;
        let batas_absensore = `${(dt.getMonth() + 1).toString().padStart(2, '0')}-${dt.getDate().toString().padStart(2, '0')}-${dt.getFullYear().toString().padStart(4, '0')} 17:00`;
        if (jam > batas_absenpagi) {
            keterangan = "absensi datang";
            absensi = 'Telat'
        } else if(jam >= batas_sore && jam < batas_absensore ) {
            keterangan = "belum waktu absen"
        } else if( jam >= batas_pagi && jam<= batas_absenpagi ) {
            keterangan="absensi pagi"
            absensi ="Hadir"
        } else if( jam >= batas_absensore ) {
        ...
        if ( keterangan !== "belum waktu absen") {
           if(kilometerjarak > batasjarak){
            res.send({ pesan: 'Jarak anda terlalu jauh'})
           }else {
            pool.getConnection(function (err, connection) {
                if (err) throw err;
            ...
            res.send({pesan: 'belum waktu absensi'})
        }
